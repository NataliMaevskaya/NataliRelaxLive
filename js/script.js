...
const popupFormulaItemHint = () => {
    const body = document.querySelector('body');//,
        // formulaItemIcon = document.querySelector('.formula-item__icon');//,
        // formulaItemPopup = document.querySelector('.formula-item-popup');
    ...
    body.addEventListener('mouseover', (event) => {
        ...
        if (target.closest('.formula-item__icon')) {// && target.closest('.row')) {
            // if (target.closest('.formula-item') || target.closest('.formula-slider__slide')) {//.formula-slider
                // if (target.closest('.row')) {
                // console.log(target.parentNode.matches('.formula-item__icon'));
                ...
                if (parentNode.matches('.formula-item__icon')) {
                    ...
                    // else {
                        formulaItemPopup.style.cssText = `visibility: visible !important;
                                                          opacity: 1 !important;`;
                        formulaItemIconInner.style.cssText = `opacity: 1 !important;`;
                    // }
                }
                // if (parentNode.matches('.formula-slider__slide')) {
                //     const formulaItemPopup = target.querySelector('.formula-item-popup'),
                //         formulaItemIconInner = target.querySelector('.formula-item__icon-inner');
                //     if(isOutOfBrowser(formulaItemPopup)) {
                //         //сделать after!?!??!
                ...
                // const formulaItemPopup = target.querySelector('.formula-item-popup');
                // formulaItemPopup.style.cssText = `visibility: visible !important`;
                // target.nextElementSibling.style.cssText = `visibility: visible !important`;
            // }
        }        
    });
    ...
    problemItems.forEach((elem) => {
        elem.addEventListener('mouseleave', (event) => {
            ...
            if (target.matches('.problems-item__icon')) {
                const problemItemPopup = target.querySelector('.problems-item-popup'),
                    problemItemIconInner = target.querySelector('.problems-item__icon-inner');
                if (problemItemPopup.getAttribute('style') && problemItemIconInner.getAttribute('style')) {
...
const slider = () => {
    ...
    sliderWrap.addEventListener('click', (event) => {
        ...
        // if(!target.matches('.portfolio-btn, .dot')) {
            if(!target.closest('.slider-arrow')){// || !target.matches('#transparency-arrow_left')) {
            return;
        }
